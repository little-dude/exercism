...
(ert-deftest is-empty-multiline ()
  (let ((s "

  "))
    (should (is-empty? s))))

(ert-deftest no-letter-is-lowercase ()
  (should (not (is-uppercase? ":) ? "))))

(ert-deftest is-question-multiline ()
  (should (is-question? ":) ?

 ")))

(ert-deftest is-question-with-whitespace ()
  (should (is-question? " what?    ")))
